...
Engine keeps model execution behind supervised adapters. The `codex` driver is
...
The adapter requires `CODEX_API_KEY` in the service environment. Engine does
...
The `pi` driver is an Engine adapter around the pinned
...
or local provider support. Engine resolves the selected catalog entry before
...
intervening edit is rejected. Engine runs approved installation and verification
...
Engine does not guarantee automatic import-graph discovery. All declared files
...
Engine’s local service cannot turn a hosted model into a local model. See
